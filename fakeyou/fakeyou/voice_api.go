...
func (f *fakeyou) GenerateTTSAudio(text string, ttsModelToken string) (*ResponseGenerateTTS, error) {
	...
	//
	resp, err := f.restyPost(routes.GENERATE_TTS_AUDIO, body)
	if err != nil {
		return nil, err
	}
	...
	// Parse the JSON response into a ResponseGenerateTTS object
	var ttsResponse ResponseGenerateTTS
	if err := json.Unmarshal(resp.Body(), &ttsResponse); err != nil {
...
func (f *fakeyou) PollTTSRequest(InferenceJobToken string) (*ResponsePollTTS, error) {
	...
	// Parse the JSON response into a ResponsePollTTS object
	var pollTTSResponse ResponsePollTTS
	if err := json.Unmarshal(resp.Body(), &pollTTSResponse); err != nil {
		return nil, fmt.Errorf("failed to unmarshal response: %w", err)
	}

	return &pollTTSResponse, nil
}
